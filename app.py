...
#
# FIX ÚNICO SOLICITADO (MÉTODO):
# - "Método" NO se guarda / aparece "No results" / se rompe el select.
# ✅ SOLUCIÓN DEFINITIVA:
#   -> Método pasa a ser SOLO TEXTO (DIGITAR) con st.text_input
#   -> Se guarda SIEMPRE como string en columna "Metodo"
...
def load_data(path: str) -> pd.DataFrame:
    ...
    if df["ID_Registro"].notna().any():
        df.loc[df["ID_Registro"].notna(), "ID_Registro"] = df.loc[
            df["ID_Registro"].notna(), "ID_Registro"
        ].astype(int)

    # ✅ MÉTODO SIEMPRE string limpio
    df["Metodo"] = df["Metodo"].fillna("").astype(str).map(lambda x: x.strip())
...
def style_table(df: pd.DataFrame) -> "pd.io.formats.style.Styler":
    ...
    return (
        df.style.apply(row_bg, axis=1).set_table_styles(
            [
...
def load_record_into_form(row: pd.Series):
    ...
    st.session_state["p1_hopt_num"] = float(row.get("Humedad_Optima_pct")) ifz pd.notna(row.get("Humedad_Optima_pct")) else 0.0
...
def control_series_by_blocks_of_3(df_f: pd.DataFrame) -> pd.DataFrame:
    ...
    for fecha_val, g in d.groupby(d["Fecha_control"].dt.date):
        ...
        for b, gb in g2.groupby("block"):
            ...
            out_rows.append(
                {"fecha": fecha_val, "block": int(b), "x_label": f"{fecha_val} (B{int(b)})", "pct_mean": pct_mean}
            )
...
<div class="qi-card">
  ...
  <div class="qi-muted" style="margin-top:8px;font-size:0.95rem">
    A={float(UMBRAL_A):.1f}% · O(usado)={float(UMBRAL_O):.1f}%
  </div>
</div>
""",
    unsafe_allow_html=True,
...
if st.session_state["PAGE"] == "P1":
    st.caption("Pantalla 1 · Ingreso + Cálculos + Tabla + Editar por ID + Eliminar por ID + Export")
    ...
    topb1, topb2, topb3, topb4 = st.columns([2.2, 1.1, 1.3, 2.4])
    ...
    with topb4:
        edit_id = st.selectbox(
            ...
            index=0,
            key="P1_EDIT_PICK",
            help="Selecciona un ID, carga en formulario y luego guarda cambios (UPDATE real).",
        )
    ...
    with c2:
        # ✅ FIX: MÉTODO SOLO TEXTO (NUNCA MÁS SELECTBOX)
        metodo_final = st.text_input(
            "Método (DIGITAR)",
            value=str(st.session_state.get("p1_metodo_txt", "")).strip(),
            key="p1_metodo_txt",
            placeholder="Ej: Densímetro Nuclear / Cono de Arena / etc.",
        ).strip()
    ...
    if (dens_h_v is not None) and (hum_v is not None) and (hopt_v is not None) and (dmcs_v is not None) and (dmcs_v > 0):
    ...
    with r2:
        kpi_card("% Compactación", f"{pct_disp:.1f}%" if pd.notna(pct_disp) else "—", f"A={float(UMBRAL_A):.1f}% · O={float(UMBRAL_O):.1f}%")
    with r3:
        kpi_card("Δ Humedad (Terreno-Proctor)", f"{delta_disp:+.2f}%" if pd.notna(delta_disp) else "—", f"Ventana ±{float(tol_hum_opt):.1f}% → {vent_disp}")
    with r4:
    ...
    with right_save:
        mode = "Modo nuevo registro ✅"
        if st.session_state.get("P1_EDIT_ID") and st.session_state.get("P1_EDIT_ROWKEY"):
            mode = f"Editando ID={st.session_state['P1_EDIT_ID']} ✅"
        st.info(f"{mode} · Método actual: **{metodo_final if metodo_final else '—'}**")
    ...
    if guardar:
        ...
        if not metodo_final:
            errs.append("⚠️ Falta Método (DIGITAR).")
        ...
        nuevo = {
            ...
            "Operador": operador,
            "Metodo": str(metodo_final).strip(),  # ✅ GUARDA SIEMPRE
    ...
    if guardar_cambios:
        ...
        if not metodo_final:
            st.error("⚠️ Falta Método (DIGITAR).")
            st.stop()

        dens_s = calc_densidad_seca(float(dh_num), float(h_num)) if float(dmcs_num) > 0 else np.nan
        pct_comp = calc_pct_comp(float(dens_s), float(dmcs_num)) if float(dmcs_num) > 0 else np.nan
        delta_h = float(h_num) - float(hopt_num)
        ventana = "OK" if abs(delta_h) <= float(tol_hum_opt) else "OBSERVADO"
        estado = estado_qaqc(float(pct_comp), float(UMBRAL_A), float(UMBRAL_O)) if pd.notna(pct_comp) else "—"
        ...
        update_values = {
            ...
            "Sector_Zona": sector_final,
            "Tramo": tramo_final,
            "Frente_Tramo": st.session_state.get("p1_frente", ""),
            "Operador": operador,
            "Metodo": str(metodo_final).strip(),  # ✅ UPDATE REAL
            "Densidad_Humeda_gcm3": float(dh_num),
            "Humedad_medida_pct": float(h_num),
            "Humedad_Optima_pct": float(hopt_num),
            "Delta_Humedad_pct": float(delta_h),
            "Ventana_Humedad": str(ventana),
            "Densidad_Seca_gcm3": float(dens_s) if pd.notna(dens_s) else np.nan,
            "DMCS_Proctor_gcm3": float(dmcs_num),
            "pct_Compactacion": float(pct_comp) if pd.notna(pct_comp) else np.nan,
            "Umbral_Cumple_pct": float(UMBRAL_A),
            "Umbral_Observado_pct": float(UMBRAL_O),
    ...
    st.subheader("Registros (Base de datos)")
    if df_all.empty:
        st.info("Aún no hay registros.")
    else:
        df_show = df_all.sort_values(["Timestamp", "ID_Registro"], ascending=False).copy()
        df_view_user = df_show.drop(columns=["RowKey"]).head(120).copy()
        ...
        sel_ids = st.multiselect("Selecciona ID_Registro a eliminar", options=ids, default=[], key="p1_del_ids")
...
# =========================================================
    ...
    with k1:
        kpi_card("Total muestras", str(k["total"]))
    with k2:
        kpi_card("Aprobación (A)", str(k["a"]))
    with k3:
        kpi_card("Observación (O)", str(k["o"]))
    with k4:
        kpi_card("Rechazo (R)", str(k["r"]))
    ...
    if df_f.empty:
        ...
        df_view_user = df_view.drop(columns=["RowKey"]).head(200).copy()
        ...
        sel_ids = st.multiselect("ID_Registro a eliminar", options=ids, default=[], key="p2_del")
